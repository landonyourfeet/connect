...
// Generate a one-time mobile login link (called from desktop)
app.post('/api/auth/mobile-token', auth, (req, res) => {
  try {
    const token = jwt.sign({ id: req.agent.id, mobile: true }, JWT_SECRET, { expiresIn: '5m' });
    const url = `${process.env.APP_URL || 'https://connect.okcreal.com'}/m?mtoken=${token}`;
    res.json({ url, token, expiresIn: 300 });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/twilio/sms', auth, async (req, res) => {
  try {
    ...
    const { to, body, personId, lineId, mediaUrls } = req.body;
    if (!to || (!body && (!mediaUrls || !mediaUrls.length))) return res.status(400).json({ error: 'Missing to, and body or media' });
    ...
    const mediaJson = mediaUrls?.length ? JSON.stringify(mediaUrls.map(u => ({ url: u, contentType: 'image/jpeg' }))) : null;
    const displayBody = body || `📎 ${mediaUrls?.length || 0} attachment${(mediaUrls?.length||0) > 1 ? 's' : ''}`;

    const actR = await pool.query(
      `INSERT INTO activities (person_id, agent_id, type, body, direction, sms_status, media_urls)
       VALUES ($1, $2, 'sms', $3, 'outbound', 'sending', $4) RETURNING *`,
      [personId || null, req.agent.id, displayBody, mediaJson]
    );
    ...
    try {
      const msgParams = { from: fromNumber, to };
      if (body) msgParams.body = body;
      if (mediaUrls?.length) msgParams.mediaUrl = mediaUrls;
      if (!body && !mediaUrls?.length) msgParams.body = '';
...
app.post('/api/twilio/sms-inbound', async (req, res) => {
  ...
  try {
    ...
    if (!From && !NumMedia) return;

    // Capture MMS media attachments
    ...
    if (mediaCount > 0) console.log(`[MMS Inbound] ${mediaCount} media attachment(s) from ${From}`);

    // Allow media-only messages (no body text)
    const messageBody = Body || (mediaCount > 0 ? `📎 ${mediaCount} attachment${mediaCount > 1 ? 's' : ''}` : '');
    if (!messageBody && !mediaCount) return;
    ...
    const actR = await pool.query(
      `INSERT INTO activities (person_id, type, body, direction, sms_status, media_urls, created_at)
       VALUES ($1, 'sms', $2, 'inbound', 'received', $3, NOW()) RETURNING *`,
      [person?.id || null, messageBody, mediaUrls.length ? JSON.stringify(mediaUrls) : null]
    );

    if (!person) {
      console.log(`[SMS Inbound] Unmatched number ${From}: "${messageBody.substring(0, 80)}"${mediaCount ? ` +${mediaCount} media` : ''}`);
    } else {
      console.log(`[SMS Inbound] From ${person.first_name} ${person.last_name||''} (${From}): "${messageBody.substring(0, 80)}"${mediaCount ? ` +${mediaCount} media` : ''}`);
    }
...
// ─── MMS MEDIA UPLOAD ─────────────────────────────────────────────────────────
const fs = require('fs');
const MMS_UPLOAD_DIR = '/tmp/mms-uploads';
try { fs.mkdirSync(MMS_UPLOAD_DIR, { recursive: true }); } catch(e) {}

// Serve uploaded MMS files publicly (Twilio needs to fetch them)
app.use('/mms-media', express.static(MMS_UPLOAD_DIR, { maxAge: '10m' }));

const mmsUpload = multer({
  storage: multer.diskStorage({
    destination: (req, file, cb) => { try { fs.mkdirSync(MMS_UPLOAD_DIR, { recursive: true }); } catch(e) {} cb(null, MMS_UPLOAD_DIR); },
    filename: (req, file, cb) => {
      const ext = file.originalname.match(/\.(\w+)$/)?.[1] || 'jpg';
      cb(null, `${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`);
    }
  }),
  limits: { fileSize: 5 * 1024 * 1024 }, // 5MB per file (Twilio MMS limit)
  fileFilter: (req, file, cb) => {
    const allowed = /^(image\/(jpeg|png|gif|webp|bmp)|video\/(mp4|3gpp)|application\/pdf)$/;
    cb(null, allowed.test(file.mimetype));
  }
});

// Upload media for outbound MMS — returns public URL for Twilio to fetch
app.post('/api/mms/upload', auth, mmsUpload.array('media', 10), (req, res) => {
  if (!req.files?.length) return res.status(400).json({ error: 'No files uploaded' });
  const appUrl = process.env.APP_URL || `https://connect.okcreal.com`;
  const urls = req.files.map(f => ({
    url: `${appUrl}/mms-media/${f.filename}`,
    contentType: f.mimetype,
    size: f.size,
    originalName: f.originalname,
  }));
  // Auto-clean uploaded files after 10 minutes (Twilio fetches within seconds)
  setTimeout(() => {
    for (const f of req.files) {
      try { fs.unlinkSync(f.path); } catch(e) {}
    }
  }, 600000);
  console.log(`[MMS Upload] ${req.files.length} file(s) uploaded by ${req.agent?.name || 'unknown'}`);
  res.json({ files: urls });
});
...
async function initDB() {
  ...
  await run(`ALTER TABLE activities ADD COLUMN IF NOT EXISTS media_urls JSONB DEFAULT NULL`, 'activities.media_urls');
...
// Mobile PWA route — embedded HTML (no separate file needed)
const _MOBILE_HTML = Buffer.from('', 'base64').toString('utf8');
// PWA manifest and icons — embedded
const _MANIFEST_JSON = Buffer.from('', 'base64').toString('utf8');
const _ICON_192 = Buffer.from('', 'base64');
const _ICON_512 = Buffer.from('', 'base64');
app.get('/manifest.json', (req, res) => { res.type('application/json').send(_MANIFEST_JSON); });
app.get('/icon-192.png', (req, res) => { res.type('image/png').send(_ICON_192); });
app.get('/icon-512.png', (req, res) => { res.type('image/png').send(_ICON_512); });

app.get('/m', (req, res) => {
  res.type('html').send(_MOBILE_HTML);
});
app.get('/m/*', (req, res) => {
  res.type('html').send(_MOBILE_HTML);
});
